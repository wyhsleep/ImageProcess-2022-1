...
int main()
{
    char readpath[] = "binary-triangle-distorted.bmp";
    char writepath[] = "out-binary-triangle-distorted.bmp";
    readBmp(readpath);
	...
	int unit[3][3] = {{0,255,0},{255,255,255},{0,255,0}};
	int sum = 0;
			...
			*(tmp_erosion+i*lineByte+j) = 0;
			{
				...
				for(int m = -1;m < 2;m++)
				{
					for(int n = -1;n <2;n++)
					{	
						sum = sum + (*(pBmpBuf+(m+i)*lineByte+n+j))*(unit[1+m][1+n]);
					}
					if(sum == 255*255*5)
					{
